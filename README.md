...
The footer shows the remaining account allowance for **Astra** and **Fable**. Hover or click a meter for the applicable limits and reset times. Each bar uses the most restrictive applicable limit; Astra shows the shared Codex allowance when no separate Astra limit is reported. Usage refreshes once a minute using the existing Codex and Claude Code sign-ins. Unavailable usage is shown as a dash, not an empty quota.

An open or draft GitHub PR for the checkout’s current branch appears as a **PR #…** link in the terminal header, in both views. Discovery uses the existing GitHub CLI sign-in and updates in the background.
