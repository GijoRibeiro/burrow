...
Coordination CLI: %s
Before creating workers, check that HEAD exists. A newly initialized repository has no commits: explain that worker worktrees need a committed starting point, help the user prepare an appropriate first commit, and do not launch workers until it exists. Creating Git metadata did not authorize committing every file. Never substitute an empty commit when workers need the existing project files.
...
func (m *Manager) preparePlan(headID string, v PlanRequest) (TeamPlan, error) {
	...
	for _, prior := range m.state.Plans {
		if prior.HeadID == headID && prior.Status == "proposed" {
			return TeamPlan{}, errors.New("an earlier team is pending launch; read plans and use start <plan-id> to launch it instead of creating duplicate assignments")
		}
